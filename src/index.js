...
function simplifiedUI() {
  let union = document.querySelector('.clearfix.union-box');
  let title = getElement('#J_roomTitle').innerText;
  let hTitle = `
    <h1 style="font-size: 2em;margin:0 20px 0 0;">
      <a href="https://www.huya.com/" class="clickstat" style="color:#000;" title="${title}">
        <span style="float: left;max-width: 400px;text-overflow: ellipsis;white-space: nowrap;overflow: hidden;">${title}</span>
      </a>
    </h1>`;
  ...
  newNode.innerHTML = '<div id="chatSwitcher" style="cursor: pointer;">' + getElement('.host-name').innerText + '<br/>订阅:<em class="subscribe-count subscribe-count-ed" id="activityCount">0</em><br/>在线:<em class="host-spl" id="live-count">0</em></div>';
  header.insertBefore(newNode, getElement('#J_duyaHdSearch'));
  // header.style.width='97%';
  header.style.margin = '0 auto';
  newNode.addEventListener('click', switchChat, false);
  ...
  if (union !== null) {
    ...
    union.remove();
    group.append(union);
    union.style.padding = '5px';
    union.style.height = '61px';
    union.style.borderRadius = '0';
    union.style.borderTop = '1px solid #eee';
    union.style.borderLeft = '0';
    union.style.borderRight = '0';
    union.style.borderBottom = '0';
    union.style.background = 'white';
...
function getPanel(id, title, content) {
  ...
  <div class="roomBlockWords">
    ...
    <i class="roomBlockWords-x" onclick="document.querySelector('#${id.replace('J_roomChat', 'J_roomChatIcon')}').click()"/>
  </div>`;
...
async function sendGift() {
  ...
  if (data !== null && data.date === getDate() && !window.confirm('今天好像已经送过虎粮了,还要送?')) return;
